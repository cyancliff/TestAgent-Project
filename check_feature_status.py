...
def check_db_status():
    ...
    try:
        ...
        if missing_columns:
            ...
            print("[HINT] 请运行: python migrate_database.py")
            return False
        else:
        ...
        # 评估状态
        if total == 0:
            print("\n[ERROR] 数据库中没有题目，请先运行: python import_data.py")
            return False
        elif with_features == 0:
            print("\n[WARN] 特征向量尚未生成")
            print("[HINT] 请安装依赖后运行: python generate_feature_vectors.py")
            return False
...
def main():
    ...
    if not db_ok and not deps_ok:
        ...
        print("   2. 导入数据: python import_data.py")
        print("   3. 生成特征向量: python generate_feature_vectors.py")
        return 1
    elif not db_ok:
        print("[WARN] 数据库状态异常")
        print("   请检查数据库和特征向量生成")
