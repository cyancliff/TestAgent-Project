...
def correlate(input1, input2):
    ...
    if spatial_correlation_sample is not None:
        ...
        out_corr = spatial_correlation_sample_fallback(input1, input2, patch_size=21, dilation_patch=2)
...
def spatial_correlation_sample_fallback(input1, input2, patch_size=21, dilation_patch=2):
    """Pure PyTorch replacement for spatial_correlation_sampler.

    This keeps MOL runnable on Windows machines without a local CUDA toolkit.
    The output shape matches spatial_correlation_sample for kernel_size=1.
    """
    pad = (patch_size // 2) * dilation_patch
    padded = F.pad(input2, (pad, pad, pad, pad))
    corrs = []
    for y in range(patch_size):
        ...
        for x in range(patch_size):
            x0 = x * dilation_patch
            shifted = padded[:, :, y0:y0 + input1.size(2), x0:x0 + input1.size(3)]
            row.append((input1 * shifted).sum(dim=1))
        corrs.append(torch.stack(row, dim=1))
