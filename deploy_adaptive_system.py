...
def run_tests():
    ...
    test_script = Path("test_adaptive_selection.py")
    ...
    print("运行智能选题系统测试...")
    try:
        result = subprocess.run([sys.executable, "test_adaptive_selection.py"],
                              capture_output=True, text=True)
...
def main():
    ...
    if choice == "1":
        ...
        subprocess.run([sys.executable, "monitor_selection.py"])
    elif choice == "6":
        show_instructions()
    elif choice == "7":
        print("退出")
